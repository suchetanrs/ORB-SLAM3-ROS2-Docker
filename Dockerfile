...
COPY traversability_mapping /root/trav_ws/src/traversability_mapping

# Build ORB-SLAM3 with its dependencies.
RUN if [ "$USE_CI" = "true" ]; then \
    . /opt/ros/humble/setup.sh && cd /root/trav_ws/ && colcon build --symlink-install && . /root/trav_ws/install/setup.sh && \
...
RUN rm -rf /home/orb/ORB_SLAM3 /root/colcon_ws /root/trav_ws
